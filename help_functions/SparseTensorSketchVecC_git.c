/*
// SPARSETENSORSKETCHVECC.C 	Computes the sketch of the vectorization of the input
//								tensor. The sketch is the TensorSketch corresponding
//								to the input hash functions h and s.
//							
// INPUTS:
...
void compute_sketch() {
	...
	for(i = 0; i < nnz; ++i) {
		...
		for(j = 0; j < no_hash_func; ++j) {
			sum += h[j][input_indices[j + i*no_dim] - 1];
			prod *= s[j][input_indices[j + i*no_dim] - 1];
		}
		target_row = (sum - no_hash_func) % sketch_dim;
		output_vec[target_row] += prod*input_values[i];
	}
}
