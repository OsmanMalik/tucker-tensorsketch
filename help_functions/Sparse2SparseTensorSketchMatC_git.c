...
//
// OUTPUTS:
// 	MsT_subs	- The subindices of a TensorSketched version of the transpose of the mode-n 
//				  matricization of the input tensor. MsT will have sketch_dim rows and 
//				  no_cols columns. Format is a matrix of size nodim x nnz.
//
//	MsT_vals 	- The values of a TensorSketched version of the transpose of the mode-n
//				  matricization of the input tensor. Format is a column vector.
//
// NOTES:
...
// 				  efficient.
//
//	Note 2 		- (Version) This function is derived from SparseTensorSketchMatC.c. The difference
//				  is that this function returns a sparse sketch instead which is better when the 
//				  sketch dimension is quite large.
*/
...
void compute_sketch() {
	...
	for(i = 0; i < nnz; ++i) {
		...
		output_vals[i] = prod*input_values[i];
		output_subs[2*i] = target_row + 1;
		output_subs[2*i + 1] = input_indices[n-1 + i*no_dim];
	}
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	plhs[0] = mxCreateDoubleMatrix(2, nnz, mxREAL);
	plhs[1] = mxCreateDoubleMatrix(nnz, 1, mxREAL);
	output_subs = mxGetPr(plhs[0]);
